"""
PRISM — Professional PDF Report Generator v2
=============================================
...
class CoverDeco:
    def __init__(self, accent, agency, client, title):
        self.accent=accent; self.agency=agency; self.client=client; self.title=title
...
class ContentDeco:
    ...
    def __call__(self, canv, doc):
        ...
        canv.drawRightString(w-MARGIN_R,h-5.5*mm,f"Prepared for: {self.client}")
        ...
        canv.setFillColor(lighten(self.accent,0.88)); canv.rect(0,MARGIN_B-4*mm,3*mm,h-MARGIN_T-MARGIN_B+4*mm,fill=1,stroke=0)
        canv.restoreState()
...
def build_executive_summary(story, S, accent, kpi_info, files, req):
    ...
    if kpi_info["is_marketing"] and ts>0:
        ...
        if tcv>0: txt+=f"The campaigns delivered <b>{fmt(tcv)}</b> conversions. "
...
def build_kpi_section(story, S, accent, kpis):
    ...
    items=[("Total Spend",fmt_inr(kpis["total_spend"]),"#e8455a"),
           ...
           ("Total Clicks",fmt(kpis["total_clicks"]),"#06b6d4"),
           ("CPC",fmt_inr(kpis["cpc"]),"#a855f7"),
           ("Conversions",fmt(kpis["total_conversions"]),"#22c55e"),
...
def build_campaign_section(story, S, accent, campaigns):
    ...
    for c in campaigns:
        rows.append([cell(c["name"][:30],S["table_cell"]),cell(fmt_inr(c["spend"]),S["table_cell_c"]),
            ...
            cell(fmt(c["clicks"]),S["table_cell_c"]),cell(fmt_inr(c["cpc"]),S["table_cell_c"]),
            cell(fmt(c["conversions"]),S["table_cell_c"])])
...
def generate_pdf(session_store, req):
    ...
    df=session_store["merged_df"]; files=session_store["files"]
    kpi_info=detect_kpis(df)
    buf=io.BytesIO()
    cover_deco=CoverDeco(accent,req.agency_name,req.client_name,req.report_title)
    content_deco=ContentDeco(accent,req.agency_name,req.client_name,req.report_title)
    cf=Frame(MARGIN_L,MARGIN_B,CONTENT_W,PAGE_H-MARGIN_T-MARGIN_B,leftPadding=0,rightPadding=0,topPadding=0,bottomPadding=0)
    nf=Frame(MARGIN_L,MARGIN_B,CONTENT_W,PAGE_H-MARGIN_T-MARGIN_B-4*mm,leftPadding=0,rightPadding=0,topPadding=0,bottomPadding=0)
    doc=BaseDocTemplate(buf,pagesize=A4,
        pageTemplates=[PageTemplate(id="Cover",frames=[cf],onPage=cover_deco),
